...
export default function TestemunhosAdmin() {
  ...
  const updateTestemunho = useMutation(api.testimonials.update);
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateTestemunho({ id: editingId, ...form });
        setToast({ message: "Testemunho atualizado com sucesso.", type: "success" });
      } else {
      ...
      setToast({ message: editingId ? "Erro ao atualizar testemunho." : "Erro ao adicionar testemunho.", type: "error" });
    } finally {
      setLoading(false);
    }
  };

  const handleEdit = (item: { _id: Id<"testimonials">; name: string; role: string; location: string; quote: string }) => {
    setEditingId(item._id);
    setForm({ name: item.name, role: item.role, location: item.location, quote: item.quote });
    setErrors({});
    setShowForm(true);
      ...
      {showForm && (
        <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">{editingId ? "Editar Testemunho" : "Adicionar Testemunho"}</h2>
          ...
          <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2 flex justify-end gap-3">
              ...
              >
                {loading ? "A guardar..." : editingId ? "Atualizar Testemunho" : "Adicionar Testemunho"}
              </button>
            </div>
          </form>
        </div>
